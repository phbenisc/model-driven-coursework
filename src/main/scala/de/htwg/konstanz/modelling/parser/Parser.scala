...
class Parser extends RegexParsers {
  ...
  def field: Parser[Field] = "Field" ~ opt("required") ~ text ~ ":" ~ dataType ^^ { case _ ~ required ~ name ~ _ ~ dataType => Field(name, required.isDefined, dataType) }
  ...
  def dataType: Parser[DataType] =
    text ~ opt(intTuple) ~ "pos" ~ intTuple  ~ opt(fixedValue) ~ opt(enumeration) ^^ { case name ~ optTuple ~ _ ~ pos ~ optFixVal ~ optEnum  =>
      DataType(
        name,
        pos._1,
        pos._2,
